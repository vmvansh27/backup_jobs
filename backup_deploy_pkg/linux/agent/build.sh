...
APP_ROOT="${APP_ROOT:-./bin/linux-x64}"
OUT="${OUT:-./dist}"
mkdir -p "$OUT"
...
cp -f "$APP_ROOT/backup-agent" pkgroot/usr/local/bin/backup-agent || echo "Place your compiled 'backup-agent' at $APP_ROOT"
...
EOF
fpm -s dir -t deb -n backup-agent -v "$APP_VER" -C pkgroot   --deb-no-default-config-files   --description "Backup Agent"   --license "Proprietary"   --maintainer "YourCo"   usr/local/bin/backup-agent etc/systemd/system/backup-agent.service
